...
def distance_pbc(box, x1, x2):
    ...
    for i in range(3):
        dx[i] = x1[i] - x2[i]
        if dx[i] > box[i]*0.50: 
            dx[i] = dx[i]- box[i] # we bring the particle back inside the box
        elif dx[i] <= -box[i]*0.50:
            dx[i] = dx[i] + box[i] #update the distance                     
...
# This computes the bonded terms 
...
def calculate_forces(box, coords, elem, elem_n, conect, exclude, ff):
    N = len(coords)

    force=np.zeros([N,3]) #empty force array
   
    [ Vbond, force ]    = bonded_forces(box, coords, elem, conect, force,
                                        ff["bond_length"], ff["bond_force_const"])
    [ Vnonbond, force ] = nonbonded_forces(box, coords, elem_n, exclude, force,
