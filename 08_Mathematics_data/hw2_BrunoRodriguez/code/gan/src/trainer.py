...
class GanTrainer():
    ...
    def objective(f, g, data_sample, noise_sample):
        ...
        """
        # TODO 
        # computation of E[f(x) - f(g(z))]
        W1 = torch.mean( f(data_sample) - f(g(noise_sample)) )           
        return W1
    ...
    def alternating_update(self, f, g, f_optim, g_optim,f_ratio=1):
        ...
        if self.step % f_ratio==0:
            f.eval()
            g.train()                                         
            g_optim.zero_grad()   #set to zero all gradients                       
            criterion = self.objective(f, g, real, noise) # this is the loss function                       
            criterion.backward() #computation of backward pass
            g_optim.step() # optimizer step                        
        else:
            f.train()
            g.eval()
            f_optim.zero_grad() #set to zero all gradients      
            criterion = - self.objective(f, g, real, noise) # this is the loss function; negative since we maximize                                          
            criterion.backward() #computation of backward pass                   
            f.enforce_lipschitz() # in the reference paper, this is done before the optimzer step 
            f_optim.step()   # optimizer step         
