...
class RMSPropOptimizer(optimizer):
    ...
    def step(self):
        ...
        for i, p in enumerate(self.parameters):
            ...
            p.grad = p.grad * self.learning_rate/(self.delta + torch.sqrt( self.r[i] ) )        
            p.data -= p.grad        
...
class AMSgradOptimizer(optimizer):
    ...
    def step(self):
        ...
        for i, p in enumerate(self.parameters):
            ...
            self.m2_max[i] = torch.max( m2_hat, self.m2_max[i] )
            
            p.grad = self.learning_rate * m1_hat/ ( self.delta + self.m2_max[i].sqrt() )
            p.data -= p.grad                        
