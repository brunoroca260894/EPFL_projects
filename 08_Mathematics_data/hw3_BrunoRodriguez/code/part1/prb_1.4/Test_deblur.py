...
# Define operators
Aoper = lambda m,n,h: comp2real(1.0/sqrt(imsize)*n*vec(fft2(Cop(m))*fft2(Bop(h)))) # b = F y = F w*x
...
# Last modification: November 26, 2019
    ...
    for iteration in range(0, maxit+1):
        ...
        # print('after svdsArg', svdsArg.shape)
        # print(svdsArg)
        ...
        # Step size
        weight = 2./(iteration + 1.)
        
        # Update A*X
        AX_t = (1.0-weight)*AX_t + weight*(AXsharp_t)
        
        # print('dim of res_cur: ', res_cur.shape)
        # print('dim of b: ', b.shape)
        # print('dim of svdsArg: ', svdsArg.shape)
        # print('dim of AX_t: ', AX_t.shape)
        # print('dim of AXsharp_t: ', AXsharp_t.shape)      
        # print('dim of topLe_vec: ', topLe_vec.shape) 
        # print('dim of topRe_vec: ', topRe_vec.shape) 
        # print('sing value: ',  singVal)
        # Update X
        X_h = -kappa* topLe_vec @ topRe_vec # this a 1-rank matrix
        X = (1.0-weight)*X + weight*(X_h) 
        
        # Show the reconstruction (at every 10 iteration)         
        if int( np.mod(iteration , 20) ) == 0:   
            U,S,V = np.linalg.svd(X,full_matrices=0,compute_uv=1)
            plotFunc(U[:,0], iteration)
    return X

# Run Frank-Wolfe's method
MaxIters = 100 #this parameter was orignally set to 200
kappa = 2000 # we try to improve the image by modifying this kappa
plotF = lambda m, ite: plot_func(m, Cop, x, ite)
