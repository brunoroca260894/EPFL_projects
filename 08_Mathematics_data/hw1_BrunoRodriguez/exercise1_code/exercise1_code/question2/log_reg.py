...
EPFL. Fall 2021. 
Those code modification for sefl-study were not implemented and set to false
when calling the corresponding methods in the main() function
...
from log_reg.algorithms import  GD, GDstr, AGD, AGDstr, LSGD, LSAGD, AGDR, LSAGDR, AdaGrad, ADAM, SGD, SAG, SVR
...
def main():
    ...
    methods['AGD'] = True
    methods['AGDstr'] = True
    methods['LSGD'] = False # point c
    methods['LSAGD'] = False # point c
    methods['AGDR'] = True
    
    methods['LSAGDR'] = False # point e
    methods['AdaGrad'] = True
    methods['ADAM'] = False #point g
    ...
    if any([methods[name] for name in det_methods + stoc_methods]):
        ...
        for key in x.keys():
            if key in det_methods:
                ax1.plot(np.array(range(info[key]['iter'])), info[key]['fx'] - f_star, color=next(colors), lw=1.5, label=key)
        ax1.legend()
        ...
        for key in x.keys():
            if key in stoc_methods:
                if key in ['GD', 'ISTA']:
                    ax2.plot(np.array(range(info[key]['iter'])), info[key]['fx'] - f_star, lw=1.5, color=next(colors), label=key, marker='o')
                else:
                    ax2.plot(np.array(range(info[key]['iter']))/float(n), info[key]['fx'] - f_star, lw=1.5, color=next(colors), label=key)
        ax2.set_xlim(0, 5)
