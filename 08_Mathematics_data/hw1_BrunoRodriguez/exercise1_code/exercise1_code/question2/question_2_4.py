...
def main():
    ...
    rate_GD = (L - mu)/(L + mu) # this is given in the HW sheet
    rate_GDmu = (L - mu)/(L + mu) # this is given in the HW sheet
    theoretical_rate_gd = lambda k: diffinit*rate_GD**(k/2)
    theoretical_rate_gd_str = lambda k: diffinit_str*rate_GDmu**(k)

    print('L constant: ', L) # the L constant >> mu
    ...
    print("GD theoretical rate: a_GD={:.6f}, b_GD={:.6f}".format(rate_GD, diffinit))
    print("GD rate diff ={}".format(a_GD-rate_GD))
    print("GD_str results: a_GD={:.6f}, b_GD={:.6f}".format(a_GDmu,b_GDmu))
    print("GD_str theoretical rate: a_GD={:.6f}, b_GD={}".format(rate_GDmu, diffinit_str))
