...
[~,ids_omega]=sort(omega_all(:,1));
figure;
plot(1:size(U,1),U(ids_omega,:).*sqrt(S(1:size(U,2)))');
xlabel('$\tilde{i}$','interpreter','latex'); ylabel('$\sqrt{\sigma_j} u_j$','interpreter','latex');
...
%visualize original and reconstructed solution (x variable)
Noscil_plot=floor(N/5);
figure;
subplot(3,1,1); plot(time_full,y_full(:,1:Noscil_plot:N),'color',[0 0 0]); hold on; xlabel('$t$','interpreter','latex'); ylabel('$(x_i)_{FOM}$','interpreter','latex');
subplot(3,1,2); plot(time_pod,y_pod(:,1:Noscil_plot:N),'color',colors(1,:)); xlabel('$t$','interpreter','latex'); ylabel('$(x_i)_{POD}$','interpreter','latex');
subplot(3,1,3); plot(time_deim,y_deim(:,1:Noscil_plot:N),'color',colors(2,:)); xlabel('$t$','interpreter','latex'); ylabel('$(x_i)_{DEIM}$','interpreter','latex');
...
%compare order parameters computed with phase and with analytical change of
%coords
figure;
plot(time_full,order_full,'color',[0 0 0]); hold on;
plot(time_pod,order_pod,'color',colors(1,:));
plot(time_deim,order_deim,'color',colors(2,:));
plot(time_full,1/N*sqrt(sum(y_full(:,1:end/2),2).^2+sum(y_full(:,end/2+1:end),2).^2),'o','color',[0 0 0]); hold on;
plot(time_pod,1/N*sqrt(sum(y_pod(:,1:end/2),2).^2+sum(y_pod(:,end/2+1:end),2).^2),'o','color',colors(1,:)); 
plot(time_deim,1/N*sqrt(sum(y_deim(:,1:end/2),2).^2+sum(y_deim(:,end/2+1:end),2).^2),'o','color',colors(2,:)); 
legend({'FOM','POD','DEIM','FOM$(x,y)$','POD$(x,y)$','DEIM$(x,y)$'},'interpreter','latex');
xlabel('$t$','interpreter','latex'); ylabel('$R$','interpreter','latex');
...
for k=1:length(latent_dim)
    ...
    %full
    elapsed_time_full_k(k)=0;
    for nsimul=1:how_many_cost
        tic;
        [time_full,y_full] = RK3 (@(t,y) rhs_fun(t,y,omega,K,N),[0 FinalTime], y0, dt);
        cost_tmp=toc;
        elapsed_time_full_k(k)=1/nsimul*(elapsed_time_full_k(k)*(nsimul-1)+cost_tmp);
    end
    phi_full=unwrap(mod(atan2(y_full(:,end/2+1:end),y_full(:,1:end/2)),2*pi),[],1);
    ...
    %POD
    elapsed_time_pod_k(k)=0;
    for nsimul=1:how_many_cost
        tic;
        [time_pod,y_pod_proj] = RK3 (@(t,y) U_pod'*rhs_fun(t,U_pod*y,omega,K,N),[0 FinalTime], U_pod'*y0, dt);
        y_pod=y_pod_proj*U_pod';
        cost_tmp=toc;
        elapsed_time_pod_k(k)=1/nsimul*(elapsed_time_pod_k(k)*(nsimul-1)+cost_tmp);
    end
    phi_pod=unwrap(mod(atan2(y_pod(:,end/2+1:end),y_pod(:,1:end/2)),2*pi),[],1);
    ...
    %DEIM
    elapsed_time_deim_k(k)=0;
    for nsimul=1:how_many_cost
        tic;
        [time_deim,y_deim_proj]=RK3(@(t,y) PTUinv*rhs_fun_colloc(t,y,PTomega,K,N,PTU,sumU,U,PTV,sumV,V,Pdeim), [0 FinalTime], U_pod'*y0, dt);
        y_deim=y_deim_proj*U_pod';    
        cost_tmp=toc;
        elapsed_time_deim_k(k)=1/nsimul*(elapsed_time_deim_k(k)*(nsimul-1)+cost_tmp);
    end
    phi_deim=unwrap(mod(atan2(y_deim(:,end/2+1:end),y_deim(:,1:end/2)),2*pi),[],1);
...
for nn=1:length(full_dim)
    ...
    %full
    elapsed_time_full_n(nn)=0;
    for nsimul=1:how_many_cost
        tic;
        [time_full,y_full] = RK3 (@(t,y) rhs_fun(t,y,omega,K,full_dim(nn)),[0 FinalTime], y0, dt);
        cost_tmp=toc;
        elapsed_time_full_n(nn)=1/nsimul*(elapsed_time_full_n(nn)*(nsimul-1)+cost_tmp);
    end
    phi_full=unwrap(mod(atan2(y_full(:,end/2+1:end),y_full(:,1:end/2)),2*pi),[],1);
    ...
    for nsimul=1:how_many_cost
        ...
        y_deim=y_deim_proj*U_pod';
        cost_tmp=toc;
        elapsed_time_deim_n(nn)=1/nsimul*(elapsed_time_deim_n(nn)*(nsimul-1)+cost_tmp);
    end
    phi_deim=unwrap(mod(atan2(y_deim(:,end/2+1:end),y_deim(:,1:end/2)),2*pi),[],1);
