...
%K=8; a=0; b=2; omega_all=a+(b-a)*rand(N,Ntrials); Kc=2/(pi*1/(b-a)); Rsqrt=@(K) nan(size(K));
...
%cauchy distribution
%K=1.8; omega_0=1; gam=0.1; omega_all = omega_0+gam*tan(pi*(rand(N,Ntrials)-1/2)); Kc=2*gam; Rsqrt=@(K) sqrt(1-Kc./K);
...
%visualize evolution of latent variables
figure;
subplot(2,1,1); plot(time_pod,y_pod_proj,'color',colors(1,:)); xlabel('$t$','interpreter','latex'); ylabel('$\alpha_{POD}$','interpreter','latex');
subplot(2,1,2); plot(time_deim,y_deim_proj,'color',colors(2,:)); xlabel('$t$','interpreter','latex'); ylabel('$\alpha_{DEIM}$','interpreter','latex');
...
%plot order parameter together with the theoretical result
%figure;
%plot([0 Kc linspace(Kc,max(K_all),100)],[0 0 Rsqrt(linspace(Kc,max(K_all),100))],':','color',[0 0 0]); hold on;
%plot(K_all,order_param_full(end,:),'color',[0 0 0]);
%plot(K_all,order_param_pod(end,:),'color',colors(1,:)); 
%plot(K_all,order_param_deim(end,:),'color',colors(2,:)); 
%legend({'Ref','FOM','POD','DEIM'},'interpreter','latex');

%save quantitites
%folder='Files_rom_oscillatory';
%filename=[folder '/' 'ku_ncoupl_' num2str(30) '_nsteps_' num2str(Nsteps) '.mat'];
%save(filename,'U','S','order_param_full','order_param_pod','order_param_deim');
